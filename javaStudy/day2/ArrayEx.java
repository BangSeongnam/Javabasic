 ...
 * 이말은 배열은 반드시 객체 변수가 있어야 하고, 인스턴스를 생성하는 코드를 new 연산자를 이용해서
 * 생성해야 한다는 뜻입니다. 
 * 배열은 P R 모든 타입을 index 의 값으로 저장할 수 있습니다.
 * 이때 이값을 기준으로 배열을 생성시에 타입을 반드시 명기 해야하는데, 같은 타입이거나 큰타입으로 타입을
 * 선언 해야합니다. 
 * 만약 큰타입으로 선언하게 되면, 내부에 들어가는 값은 해당 타입으로 promotion 되어져서 들어가게 됩니다.
 * 배열은 객체이기 때문에 배열 index 의 값이 R type 인 경우엔, 해당 instance 의 reference 가 
 * 값으로 들어가게 됩니다. (이건 도식으로 설명 할게요)
 * 자바의 배열은 매우 엄격하게 관리되어집니다. 예를들어 스크립트의 경우, 배열의 값을 동적으로 넣고 빼고
 * 할수 있지만(undefined 로 처리되는경우 포함), 자바의 배열은 위의 경우를 모두 예외로 인식하여 에러를 발생시킵니다.
 * 
 * 필수로 알아두세요!! 기본객체라고 하는데, 모든 객체를 default 로 생성하게 되면, 그 내부의 속서으이 값은 모두 각 타입의 
 * default 값으로 초기화 되어져서 생성됩니다. 
 * 
 * 배열을 예로 들면, 만약 int 타입으로 빈 배열을 생성하면, 각 index 의 값은 정수의 기본값인 
 * 0 으로 초기화 되어져 들어갑니다. 
 */
public class ArrayEx {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 */
		int[] intArr = new int[3]; // size 가 3 인 기본배열 생성함.
...
//		System.out.println(intArr[3]);
		...
		//for in 구문은 위 자료형들의 길이에 맞게 자동으로 loop 를 돌면서 값을 리턴해주는 for 구문입니다.
		...
		//그 클래스 이름은 java.utill.Arrays 라는 애 입니다. 
		...
		//배열생성3. 1번과 2번을 합친 형태입니다. 
		//이 배열식은 동적으로 배열을 생성할때 사용됩니다. 특정 문법에서는 이 방법만 쓸수 있을때가 있어요...
		//지금은 문법만 기억 하길.
