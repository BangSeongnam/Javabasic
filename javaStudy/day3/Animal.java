...
//이 클래스는 객체 지향의 예를 위한 클래스입니다.
//이름에서도 볼수 있듯이, 동물을 정의하도록 할 예정입니다.
/*
 * 클래스에는 속성(객체가 발생했을때, 그 객체의 상태를 나타냄, 자바에서는 이를 MEMBER FIELD 라고함)
...
//	private int age;
	...
	//VM 이 해당 클래스에서 찾는, 실제 instance 를 생성하도록 정의 된 일종의 메서드 입니다.
	//때문에 () 가 있는것이죠. 하지만, 메서드(기능)과는 구분이 되어야 하기 때문에 아래와 같은 규칙을 갖습니다. (꼭 기억 하세요)
	/*
	 ...
	 * 5. 만약 4번과 반대의 경우, 즉 생성자를 하나 이상  정의시에는 컴파일러는 기본 생성자를 넣어주지 않습니다.
	 * 6. 잘 이해하면 여러분에게 큰 도움이 되는 내용.
	 * 생성자는 위의 기능외에, 특정 클래스 인스턴스화 되는데의 guide 가 될수 있습니다.
	 * 이말은 정의되지 않은 생성자를 호출할수 없기 때문에, 정의된 생성자를 통해서만 객체를 발생할 수 있다는 의미가 됩니다.
...
//	}
	//생성자에서의 this([params]);
	/*
	 * 생성자에서 this() 를 사용할수 있다. 기존에 정의된 생성자를 호출할때 사용하는데,
	 * 이때 중요한건 호출 위치이다. 반드시 코드 첫머리에만 허용된다.
	 */
...
//	
	...
	//멤버메서드를 정의합니다.
	//이 메서드는 이 클래스를 바탕으로 생성된 모든 인스턴스가 가지게 됩니다. 때문에 멤버 메서드 라고 합니다.
	//메서드를 저으이 할땐 다음과 같은 문법을 따릅니다. 꼭 기억하세요.
	//[접근 제어자 public > protected > default (아무것도 쓰지않음) > private] retyrnType(P or R or void)
...
//		String aType ="이건 지역변수값";
//		//aType타입을 리턴시킵니다.. 타입 변수엔 값이 들어가 있기 때문입니다.
...
//	 }
	 ...
	 * 
	 * 이런 추상 메서드를 하나 이상 보유한 클래스는 명시적으로 추상 클래스로 선언 해야 하며, 안그럼 에러이다.
	 * 
	 * 또한 이런 추상 클래스를 상속받은 자식 클래스는 반드시 추상 메서드를 오버라이드 해야하고, 그렇지 않은 경우, 자식 클래스 또한 
	 * 추상 클래스로 선언이 되어야 한다.
	 */
